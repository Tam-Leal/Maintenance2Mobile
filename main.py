...
class MyApp(MDApp):
    ...
    def queue(self):
        ...
        if email == self.email_admin and password == password:
            ...
            for i in range(len(request_dict)):
                self.check_checkbox = self.root.get_screen('main').ids.container.add_widget(
    ...
    def graph(self):
        ...
        if email == self.email_admin:
            ...
            self.root.get_screen('main').ids.tela3.add_widget(self.piechart)

            self.report = MDLabel(
                text="Reported Issues:",
                pos_hint={"center_x": 0.62, "center_y": 0.2},
                size_hint=[None, None],
                size=(dp(300), dp(300)),
                font_style='H5'
            )
            self.root.get_screen('main').ids.tela3.remove_widget(self.report)
            self.root.get_screen('main').ids.tela3.add_widget(self.report)

            items_dict = LogData().stat_requests(user_email=email, user_password=password)
            self.piechart.items = items_dict[0]
            self.report.text = f'Reported Issues: {items_dict[1]}'
        else:
            pass
